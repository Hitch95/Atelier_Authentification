...
// Générer un jeton aléatoire pour la session
...
if (isset($_COOKIE['authToken'])) {
    if (str_starts_with($_COOKIE['authToken'], 'admin_')) {
        header('Location: page_admin.php');
        exit();
    } elseif (str_starts_with($_COOKIE['authToken'], 'user_')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === 'admin' && $password === 'secret') {
        $token = bin2hex(random_bytes(16));
        setcookie('authToken', 'admin_' . $token, time() + 3600, '/', '', true);
        header('Location: page_admin.php');
        exit();
